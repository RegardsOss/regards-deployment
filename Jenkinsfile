  ...
  stages {
    ...
    stage('Package') {
      steps {
        ...
        sh 'export MAVEN_M2=~/.m2 && mvn -V -U -P install,CI clean package org.jacoco:jacoco-maven-plugin:0.7.7.201606060606:prepare-agent sonar:sonar -Dsonar.jacoco.reportPath=${WORKSPACE}/jacoco-ut.exec -Dsonar.jacoco.itReportPath=${WORKSPACE}/jacoco-it.exec -Dsonar.branch=develop -DcmdLineTarget=target'
      }
      post {
        success {
