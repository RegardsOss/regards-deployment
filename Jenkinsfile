...
pipeline {
  ...
  stages {
    ...
    stage('Deploy Nexus') {
        ...
        steps {
            sh 'mvn -V -U -P install,CI clean deploy -Dmaven.test.skip=true -DcmdLineTarget=target -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true -Dmaven.wagon.http.ssl.ignore.validity.dates=true'
        }
    }
  }
}
